...
function App() {
    ...
    const [lists, setLists] = useState(DB.lists.map(list => {
        list.color = DB.colors[list.colorid].name
        list.click = () => {setActiveList(list.id);}
        list.active = list.id === activeList;
        return list;
    }));

    const onAddList = list => {
        const new_lists = [...lists, list];
        setLists(new_lists);
    }
  return (
    <div className="todo">
